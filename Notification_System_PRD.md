...
**Role granularity used below:** within a `team`-kind role, this system distinguishes an **admin** tier (`ngo_admin`, `gov_admin`) from a **member** tier (`ngo_member`, `gov_member`). This isn't a new RBAC concept — it's the same `roles.kind='team'` + `user_role_assign` mechanism, just with two role rows per org type instead of one. The distinction matters because several notification types are admin-only, not team-wide.

| Type | DB Trigger | `ref_type` | Priority | Recipient scope (see §4) |
|---|---|---|---|---|
| `zone_assigned` | New row in `team_zone_assign` | `work_zone` | Urgent | `team_admin` — **only `ngo_admin`** of the assigned team. NGO members can still *view* the zone/tickets on the map/ticket page (existing view permission) — they just don't get the notification. The admin is expected to triage and delegate to members via individual task assignments (which do notify — see `task_assignment_created`). |
...
| `task_assignment_created` | New row in `task_assignments` | `ticket_task` | High | `own` — the `actor_uuid` on the new assignment (this is how an `ngo_member` actually learns they have work, since they don't get `zone_assigned`) |
| `dedup_flag_ticket` | `ticket_tasks.is_duplicate` set true / `dedup_group_id` populated | `ticket_task` | Medium | `permission` — holders of a dedup-management capability (see §4) |
| `dedup_flag_station` | `stations.is_duplicate` set true / `dedup_group_id` populated | `station` | Medium | `permission` — same dedup-management holders |
| `resource_station_updated` | New row in `stations`, or update to a station's operational fields (`is_open`, `water_level`, `beds_available`, `supply_rationed`, or similar type-specific property in `station_properties`) | `station` | Medium | `team_type:gov` — **every** `gov_admin` and `gov_member`, across all Gov teams. Unlike zone assignment, this is intentionally team-wide (not admin-only) because field-level Gov staff need the same situational picture as Gov admins. See Open Question 7 on whether this should be narrowed to Gov teams whose `work_zones` overlap the station. |
...
**NGO/Gov page-access note (context, not a new requirement):** `ngo_admin` has edit+view on the resource station management page; `ngo_member` and `gov_*` are view-only there. This is why `resource_station_updated`'s *actor* is typically an `ngo_admin` (or system/crawler `source`), while its *recipients* are Gov — actor and recipient set are never assumed to be the same people (see §4).
...
| `team_admin` *(new, notification-specific)* | Recipient set = `SELECT u.uuid FROM users u JOIN user_role_assign ura ON ura.user_uuid = u.uuid JOIN roles r ON r.uuid = ura.role_uuid WHERE u.team_uuid = :team_uuid AND r.name = :org_type || '_admin'` — narrows `team` down to just the admin-tier role for that org type (`ngo_admin` or `gov_admin`). |
| `team_type:<gov\|ngo>` *(new, notification-specific)* | Recipient set = every user (admin + member) belonging to *any* team where `teams.type = 'gov'` (or `'ngo'`) — crosses team boundaries, unlike `team`/`team_admin` which are scoped to one specific team. Used for `resource_station_updated`, where the audience is "all of Gov," not one Gov office. |
| `permission` *(new, notification-specific)* | Recipient set = every user holding a given capability key, via the union of `role_permission_assign` (through `user_role_assign`) and `user_permission_assign` — e.g., whoever holds `dedup.manage`. This reuses existing RBAC tables; it does not add a new grant mechanism. |
