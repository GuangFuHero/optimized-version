...
> **日期**：2026-06-10
> **關聯**：[`feature.md`](../feature.md)（01-auth）、`AC-FEAT-001`、`IAM-FEAT-003`
...
兩套不該混用同一條登入路徑，但**同一個人可能兩邊都是**（例如某 NGO 志工先用 Line 註冊成為一般使用者，之後升等為後台 NGO 角色——見 `IAM-FEAT-003` 升等流程）。這就是「帳號連結 / 身份統一」必須先想清楚的原因。
...
| SSO 服務中斷 | 是否提供 break-glass 後備登入？ | 🟡 待確認：災害時 SSO 掛掉怎麼辦？建議至少保留 1 組平台原生的緊急 Super Admin 帳號（見 `AC-FEAT-001` 雙簽/break-glass 討論） |
