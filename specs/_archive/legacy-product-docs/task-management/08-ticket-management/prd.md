...
>
> **`tickets.task_type` 已廢除（decisions.md D9）。** 它與 `ticket_tasks.task_type` 同名同義、只是底下 Task 的重複摘要。地圖圖示與統計改由該 Ticket 的 Task 集合推導。
...
**用途：承載型別專屬與災害專屬情報**（如物資品項清單、結構穩定度、受困人數、危害物質…）。**已定調走鍵值（EAV）而非為每種災害新增固定欄位**，換來「一套骨架承載所有災種」的彈性。

「哪個需求種類可以有哪些欄位」由**欄位清單**（`task_property_config`：`task_type` / `property_name` / `data_type` / `enum_options`）定義，見 F8。
...
* 技能媒合、地理自動媒合列入後續版本。
* **志工身份模型**（承接者是「前台一般使用者」還是登錄的 Team Member）需與 [[05-member-management]] 對齊，見「待拍板 D」。
...
* SLA（承接時限）與逾時自動升級警示屬進階治理機制，是否於本版納入見「待拍板 B」。
...
* **重複偵測（合併同一件事）** 與「同建築多筆 Ticket 的群組化顯示」是不同概念；後者是否納入見「待拍板 C」。

### F6. 表格與統計

* **任務統計**：任務總量、依狀態／需求種類／Team／行政區交叉統計（需求種類取自 Task 層；Ticket 層的重複欄位已廢除，見 D9）。
...
> 畫面見 [`wireframe/02-field-admin-options.md`](wireframe/02-field-admin-options.md)、[`wireframe/03-form-effect.md`](wireframe/03-form-effect.md)。決策理由見 [`decisions.md`](decisions.md) D10–D16。

三層結構，權責由下而上收緊：

| 層 | 是什麼 | 誰能改 |
|---|---|---|
| **系統欄位庫** | 全系統可用欄位的目錄（名稱、型別、選項） | 🔒 工程，隨系統出貨 |
| **災害欄位組** | 每個災害種類對應的 `(需求種類, 欄位)` 配對集合 | 🔒 工程，隨系統出貨 |
| **本場欄位清單** | 這場應變實際生效的欄位（`task_property_config`） | ✅ Super Admin |

* **需求種類固定**（`hr` / `supply` / `rescue`），新增需工程處理。欄位一律掛在需求種類底下。
* **加開災害種類 → 該災害的欄位組自動併入本場欄位清單。** 例如地震應變中途發生水災，加開水災即帶入積水深度、是否斷電、需要抽水機、需要沙包等欄位。
* **只加不減：關閉災害種類時，已併入的欄位不會自動移除。** 欄位不得在有人正在填寫時消失。關閉時可提供「一併停用該組欄位」的一鍵選項，但**預設不勾**。
* **手動新增欄位一律先搜尋系統欄位庫，找不到才自訂。** 這是自動整併的前提：手動加入與欄位組帶入的欄位若共用同一識別，重複時自動視為同一欄位，已填資料不需搬移。
* **自訂欄位在本場應變內與欄位庫欄位完全平等**（可篩選、可設必填、可做本場統計），不會自動回饋至欄位庫；經評估可由工程升格收錄，供後續應變使用。
* **欄位可停用、不可刪除。** 停用後新表單不再顯示，已填資料仍可檢視（標為已停用、唯讀），可再啟用。
* **欄位只有「必填 / 選填」兩種狀態，不支援條件式必填。** 需要引導時以提示呈現（如樓層非 1 樓時提示「建議補齊結構評估」），不擋儲存。
* **自訂欄位一律不出現在前台民眾報案表單**，僅用於後台代報與補件（見 F1）。
* 修改欄位設定（套用災害組、新增、停用）**限 Super Admin**。
...
* [ ] 加開災害種類時，該災害欄位組自動併入本場欄位清單；關閉災害時欄位預設不移除。
* [ ] 新增欄位時先搜尋系統欄位庫；自欄位庫選取的欄位與後續災害組帶入者自動視為同一欄位，不重複建立。
* [ ] 欄位可停用、不可刪除；停用後新表單不顯示，已填資料仍可唯讀檢視。
* [ ] 欄位設定（套用災害組／新增／停用）僅 Super Admin 可執行。
* [ ] 自訂欄位不出現在前台民眾報案表單。
...
**B. SLA 與逾時升級。** 四級優先級已有，但 schema **沒有** SLA 時限、逾時升級相關欄位。要決定 v1 是否納入「生命危急逾時未承接自動升級警示」，或列入後續版本。

**C. 同建築群組化（Building Anchor）。** 同地址多樓層多筆 Ticket 合併為建築錨點顯示的機制，schema **完全沒有**對應資料表（Task 層的 dedup 是「合併同一件事」，與此不同）。要決定是否納入；納入即需新增資料表與關聯。

**D. 志工身份模型。** 承接 Task 的志工是「前台一般使用者」還是需登錄的 Team Member？影響承接權限與資料邊界，需與 [[05-member-management]] 對齊。

**E. 自訂欄位升格的維運責任。** F8 允許自訂欄位「用得好可升格進系統欄位庫」，但**跨場次的事目前無人負責**：誰決定要升格、什麼時機執行、審查標準為何。不定案則升格永遠不會發生，自訂欄位實質等同一次性。

**F. 欄位級的訪客揭露分級。** F4／[[10-guest-ticket-privacy]] 規定欄位揭露分級，但欄位清單沒有欄位級的可見性設定。自訂欄位預設對訪客可見或不可見？誰能調整？**預設不可見較安全**，但需與 10 一併定案。

### 待工程評估（非產品決策）

以下由本輪拍板衍生，屬實作面，記錄於此以免遺失：

* **廢除 `tickets.task_type` 後的地圖效能**（D9）。地圖圖示與統計改由 Task 集合即時推導，災時單一行政區可能上千至上萬點，需評估是否需要快取或反正規化摘要欄位。
* **欄位設定的權限歸屬**（D14）。現況用 `check_permission(info, "map", "edit")` 守門，與編輯地圖標記共用權限，任何能編輯標記者都能改全平台欄位設定。需改為 Super Admin 專屬。
* **RBAC 命名落差。** 程式碼採 `(resource, action)` 模型（`map` + `edit`），`_shared/engineering/rbac-permissions-design.md` 列的是 `map:marker:edit`／`map:config` 這套字串，兩邊對不起來。
* **欄位清單缺的欄位**（F8 依賴）。`task_property_config` 目前只有 `task_type`／`property_name`／`data_type`／`enum_options`，缺必填旗標、顯示名稱、排序、停用旗標、驗證規則、異動紀錄。
* **`data_type` 詞彙不一致。** 種子資料用 `Integer`／`String`／`Enum`／`Array`／`Boolean`／`Text`，GraphQL 型別描述寫的是 `'string'`／`'integer'`／`'float'`／`'enum'`，兩套對不上。
