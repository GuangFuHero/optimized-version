...
畫面上只畫了救援／人力／物資三種需求種類，實際有**四種**——還有一個**醫療**。醫療的出廠欄位已於 D30 拍板：必填只有「傷患人數」與「最嚴重的人現在」（五個觀察式選項，含「不確定」），另加兩個選填。**不使用檢傷分級**，理由見 [`../scenarios.md`](../scenarios.md) 的 S-14、S-15。明細見 [`../spec.md`](../spec.md) 的 `TM-CF-120`。要重畫這些圖時請補上第四個頁籤。
