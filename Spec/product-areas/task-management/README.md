...
| [TM-FEAT-007 Professional on-scene assessment](./features/TM-FEAT-007-professional-assessment/feature.md) | Draft | [v0.2.0](../../versions/v0.2.0.md) | How trained responders actually report on scene is unobserved; nothing should be designed before that is answered. |
...
- The medical starting field set approved in D30 asks only for observations, so it needs no clinical training to complete. The wording of its five condition choices should still be checked with a trained responder before first release, to confirm that a reader recovers the intended triage level from each one.
