...
> **日期**：2026-06-10
> **關聯**：[`feature.md`](../feature.md)（03-user-settings）、`IAM-FEAT-001`、`MEM-FEAT-001`、`IAM-FEAT-002`
...
| **防枚舉** | 無論帳號是否存在都回「若帳號存在已寄出重設連結」 | 採之（與 `IAM-FEAT-001` 防枚舉一致） |
...
> **重要區分**：03 寫的是使用者**自行**「停用」→ 對應 **Deactivate（可逆休眠）**，不是 Delete。管理者端的 **Suspend** 已在 `MEM-FEAT-001`（Team 內暫停 / 整帳號暫停）定義，兩者應對齊用語避免混淆。
>
> 🟡 **建議**：
> - 自助「停用」＝可逆休眠：登出全部 session、不再收通知，重新登入即自動恢復（或顯示「重新啟用」按鈕）。
> - 若要提供**刪除**（個資法/GDPR 刪除權），建議 **30 天寬限期 + 軟刪除**，期間可撤回，期滿才匿名化；但**最後一位 Super Admin 不可刪除**（與 `MEM-FEAT-001` E8 一致）。
> - 災害應變期間的後台人員自助停用，建議若其為 Team 唯一 Admin 須先指派接班（與 `MEM-FEAT-001` E1 一致）。

---

## 5. 角色升等申請（狀態機）

03 寫「申請送出進入審核佇列」，但缺申請的完整生命週期。對齊 `MEM-FEAT-001` F5（加入 Team → Team Admin 審；平台角色 → Super Admin 審）後，建議狀態機：
...
| **同時兩種申請** | 加入 Team 與升平台角色互不相依、可並行（已由 `MEM-FEAT-001` F7 / E4 定案） |
| **通知** | 每次狀態轉移皆透過 `IAM-FEAT-002` 站內通知告知申請人 |
| **進度可視** | 設定頁顯示目前狀態與時間軸 |
...
- 與本 repo 既有：`MEM-FEAT-001`（suspend / 最後一位 Admin/Super Admin 約束）
