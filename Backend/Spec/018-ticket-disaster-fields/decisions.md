...
以下兩條來自 2026-09-19 的 PII 範圍盤點（`PII_SCOPE_VERIFICATION.md`）。

### ADR-281 通報單的座標納入 `ticket.view_pii`，沒權限回 `null`

**白話**：地址擋了、座標卻精確公開，等於沒擋。座標跟地址放在同一道門後面。

**Context**：實測不帶 `Authorization` 查詢 `ticket` / `tickets`，`geometry` 回
`[121.4312345, 23.6654321]`（小數第 7 位，約 1 公分），同一筆的 `contactName` 是 `王◯◯`、
`secondaryLocation` 是 `null`。沒有 `ticket.view_pii` 的人匯出，經緯度也是原值。
ADR-268 擋地址的理由是「那是通報者自己的家」，但座標指的是同一棟房子；ADR-254 則是
把「座標公開」當前提在推論，沒有人決定過座標本身該不該公開。016 的分級表
（`Spec/016-resource-history/decisions.md` §PII 層）又把「精確座標」列為 PII，
兩份規格互相矛盾。

**Decision**：`TicketType.geometry` 改成 gated resolver，沿用 `_pii_visible`（與
`contact_*`、兩個檢傷欄位、`secondaryLocation` 共用同一次 scope 檢查）；沒權限回 `null`。
批次匯出的 `latitude` / `longitude` 跟著同一個逐列判斷，沒權限留空。
**站點座標不動** —— 收容所、物資站本來就要公開在地圖上（ADR-268 同一個理由）。

➕ 保護對象一致：地址、座標、聯絡方式、檢傷答案，四者都是同一個權限、同一次判斷。
➕ 匯出留空可以安全匯回：通報單的座標只在新建時寫入（`bulk_columns.py` 的 `_create_only`），
更新列本來就不會動到座標；而會留空的列，聯絡欄位也同時被遮罩，已經被 ADR-109 擋下。
➖ **公開地圖上看不到求助點**：訪客、一般志工、工作區外的隊伍在地圖上都不會出現通報單。
這是使用者 2026-09-19 的裁定（三個選項中選了「完全不給」，否決「降精度」與「維持公開」）。
➖ 否決「降精度到約 100～500 公尺」：模糊過的座標仍然是位置資料，而且鄉間一格
可能只有一兩戶，等於沒模糊。
◾ 本條取代 ADR-254 Context 裡「地圖上的座標也是公開的」這個前提；ADR-254 的決定本身不變。

### ADR-282 `tickets(bounds:)` 只篩呼叫者看得到座標的通報單

**白話**：座標拿掉了，框選範圍還能用的話，把框一直縮小就能把位置推回來。

**Context**：ADR-281 讓沒權限的人讀不到 `geometry`，但 `tickets(bounds:)` 用
`ST_Intersects` 篩選。呼叫者只要反覆縮小範圍、看某張單還在不在結果裡，就能把被隱藏的
點找回來 —— 欄位擋了，查詢條件沒擋，等於沒擋。

**Decision**：帶了 `bounds` 時，額外套上呼叫者 `ticket.view_pii` 的 `scope_filter`
（own → 自己建的；zone → 工作區內；all → 不限；沒有 → `false()`）。
**不帶 `bounds` 的列表完全不變**：公開求助版照樣列出所有通報單（ADR-027），只是
`geometry` 為 `null`。`count` 與 `list` 用同一組條件，`totalCount` 不會透露被濾掉幾筆。

➕ 一條規則涵蓋所有角色：沒有「訪客直接報錯、own/zone 另外過濾」兩套邏輯
（使用者 2026-09-19 裁定，否決「沒權限就拒絕 bounds」）。
➕ 有權限的救援隊地圖不受影響：框選結果就是他們本來就看得到座標的那些單。
➖ 訪客帶 `bounds` 會拿到空結果而不是錯誤，前端要知道「空」可能代表「沒有權限」。
◾ `q` 關鍵字搜尋不受影響：它比對的欄位（標題、描述、任務）都是公開的，地址早在
ADR-146 就排除在索引外。
