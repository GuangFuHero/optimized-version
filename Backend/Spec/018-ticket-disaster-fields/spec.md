...
- 通報單可以記錄與修改地址(在此之前只有站點可以),讀取需要 `ticket.view_pii`。

## 非目標（YAGNI，明確排除）

- **不做前端動態表單渲染。** `ticket-create-drawer.tsx` 仍是靜態表單,本功能只提供後端契約。
- **不做欄位值驗證。** 沿用 ADR-092:設定表只是渲染提示,後端不檢查寫入值的型別或選項。
- **不限制 `disasterDetails` 的鍵,讀取維持公開。** 設定表不當白名單,否則表單載入後才被
  retire 的欄位會弄丟受困者的答案(ADR-279)。量已由 ADR-267 封住;若日後建出自由文字的
  通報單欄位,它的內容預設就是公開的。
- **不做 `min_value` / `max_value`。** 既然沒有任何東西會驗證,「>=0」留在 `hint` 文字即可。
- ~~**不做通報單地址的讀取端。**~~ PR #50 第二輪審查改為納入範圍:地址可以修改,
  讀取則由 `ticket.view_pii` 把關(ADR-268)。原本延後的 ADR-146 判斷就此做出。
