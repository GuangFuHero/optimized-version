...
> **實作註記**：排序先以「主表 `search_text` 自己是否命中」的布林分成兩組，命中的整組排
> 在前面；`similarity()` 只決定組內順序（ADR-147 取代 ADR-083 的單鍵排序）。這是刻意的
> ——名稱叫「發電機站」比「擁有一台發電機」更相關。不能只靠 `similarity()`：`pg_trgm`
> 需要 padding 才組得出 trigram，中文關鍵字若不在文字開頭就恆為 0，「花蓮縣光復鄉救災站」
> 搜「光復」得 0，與「只靠關聯表命中」無法區分。
