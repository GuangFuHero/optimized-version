...
- [ ] **Step 1: 加 extension 到測試 bootstrap**
...
- [ ] **Step 2: 確認 `tests/test_graphql/conftest.py` 共用同一個 bootstrap**
...
- [ ] **Step 5: `secondary_locations` 欄位語意註解**
...
- [ ] **Step 2: repository 加 `q` —— `list_active` 與 `count_active` MUST 共用同一個 predicate builder**
...
- [ ] **Step 1: 確認 operator class 與查詢相容**
...
- [ ] 6 個 `search_text` 的欄位組成逐一比對 `spec.md` §3——**沒有任何 `contact_*` 或 `comment` / `progress_note` / `pole_note`**
- [ ] `tests/test_search_schema.py` 的 `EXPECTED_SOURCE_COLUMNS` 與 migration 的 `_SEARCH_TEXT` 一致
- [ ] **`list_active` 與 `count_active` 的本體中不存在任何條件組裝**，兩者的條件都來自 `_active_conditions()` 的單一呼叫
- [ ] `build_search_condition` 的第一個欄位是位置必填參數（無欄位呼叫會 `TypeError`）
- [ ] wildcard escape 對 `%`、`_`、`\` 三者皆有測試，且含混合案例
- [ ] migration 為手寫，未使用 autogenerate；可 `downgrade -1` 後再 `upgrade head`
- [ ] 未新增 GraphQL error masking extension 或自訂 handler（沿用既有 `raise ValueError` 慣例）
