...
### ADR-224 新增聯絡方式也要通知帳號既有的管道

**白話**：帳號多了一個可以登入、可以收密碼重設信的東西，既有的管道應該要知道。

**Date**: 2026-09-06（PR #39 第三輪 review 後補）

**Context**：`commit_contact_change` 的取代分支一直都會通知舊管道——ADR-085 說得很明白，那是
**受害者唯一能察覺的機制**。但 `existing is None` 那條分支（該型別的第一個）直接 `return False`，
不通知任何人。

而那個被加進來的值**是登入識別碼，也是 `forgot-password` 的目的地**。所以「帳號能被誰進入」這件事
變了，變的幅度跟取代一樣大，卻是無聲的。ADR-215 對 `set-password` 用的正是相反的推理：
「SSO-only 帳號多一把永久鑰匙這件事不可以無聲發生」。第一個某型別的聯絡方式是同一類事件。

**Decision**：`_notify_contact_added`——插入成功後，通知**帳號原本就有的**每一個管道。
新加的那個不通知：它剛剛才證明自己收得到信，通知它對擁有者沒有任何意義。

第一個聯絡方式（帳號原本一個都沒有）不通知任何人——沒有人可以通知，也還沒有東西要保護。

**Consequences**：
➕ ADR-085 的「通知是唯一的偵測機制」現在涵蓋新增，不只涵蓋取代。
➕ 與 ADR-218（登入方式變動通知）湊成完整的一組：**任何改變「誰能進入這個帳號」的動作都會被看見**。
➖ 通知量增加。都是低頻的安全事件。
➖ 通知在插入之後才送，寄送失敗只記 log（ADR-162）——與其他所有通知一致。

---

### ADR-225 step-up 寄送計數的 TTL 每次都設，不只在第一次

**白話**：那個防濫寄的計數器如果掉了到期時間，這個帳號就永遠不能再改聯絡方式了。

**Date**: 2026-09-06（PR #39 第三輪 review 後補）

**Context**：`issue_old_channel_step_up` 用 `INCR` 建立 `stepup_sends:{uuid}:{type}`，
而 **`INCR` 建出來的 key 沒有到期時間**；TTL 是下一行 `expire` 補的，而且**只在 `sends == 1` 時執行**。

兩個 round trip 之間如果連線斷掉或 worker 死掉，那個 key 就永遠留著。而它在整份檔案裡
**從來不會被刪除或重設**——`discard_old_channel_step_up`（ADR-216）刻意不動它。於是累積到
`MAX_STEPUP_SENDS_PER_WINDOW` 之後，這個帳號**永久 throttled**：那個型別的聯絡方式再也不能取代或
移除，而且自 ADR-215 起也再也不能設定第一組密碼。

這與 ADR-216 修掉的是同一類失效（一個暫態狀況活得比它該有的窗口久），只是這一個**不會隨著 OTP
視窗結束而自癒**。

**Decision**：`await self.redis.expire(sends_key, self.ttl, nx=True)`，**無條件每次都跑**。

- `nx=True` 讓已經有 TTL 的 key 不被延長——所以這仍然是 ADR-165 指定的**固定窗口**，不會變成滑動窗口。
- 因為無條件執行，**下一次呼叫就會把掉了 TTL 的 key 修回來**，不需要任何清理工作。

**Consequences**：
➕ 計數器不可能活得比它要限制的那個窗口久。
➕ 自癒：即使真的發生了那個競態，下一次寄送就補上 TTL。
➖ 每次寄送多一次 `EXPIRE` round trip。相對於一次寄信/簡訊可以忽略。
➖ 需要 Redis 7.0+（`EXPIRE ... NX`）。dev 與 staging 的 compose 都是 `redis:7-alpine`。
