...
停用的欄位不回傳：查詢預設追加 `AND is_active = true`，管理端可用 `includeInactive: true` 取回已停用欄位（需 `dynamic_field.edit`，ADR-226）。排序改為 `ORDER BY sort_order, property_name, uuid`（以 `uuid` 收尾才是全序，ADR-227）——目前完全沒有 `ORDER BY`（`app/repositories/config_repository.py:17-26`），前端拿到的順序不保證穩定。
