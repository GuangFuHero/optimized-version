...
- **權限**：`Perm.USER_EDIT`，checkpoint 1 only，且**範圍必須是 `Scope.ALL`**（ADR-103 定權限、ADR-107 補範圍條件——初版寫「沿用既有 scope 判定」，實作時更正為 checkpoint 1 only，2026-08-25 review 再補上 `ALL` 的要求）。
...
| `app/core/security.py` | `get_current_user` 增加 redis 參數與 session 檢查；Redis 故障的 log。`get_current_session` 同樣加上（ADR-106，2026-08-25） |
...
> **2026-08-25 更正**：`logout` / `logout-all` 的**函式本體**確實沒改，但它們依賴的 `get_current_session` 加上了檢查（ADR-106）——原本那兩個端點是撤銷後唯一還打得動的路徑。docstring 一併更正。
...
| 權限 | 持 `USER_EDIT` 但範圍非 `all`（own/team/gov/ngo/zone）呼叫踢人 → 403（ADR-107） |
| 安全 | 已撤銷的 token 呼叫 `logout` / `logout-all` → 401（ADR-106） |
| 安全 | 沒有 `sid` 的 token 呼叫 `logout` → 401（不再是 no-op，ADR-106） |
