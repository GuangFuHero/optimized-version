...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("actor_uuid", sa.UUID(as_uuid=True), nullable=True, comment="觸發此事件的使用者 UUID (系統觸發為 NULL)"),
        sa.Column("type", sa.String(length=50), nullable=False, comment="通知類型 enum"),
        sa.Column("priority", sa.String(length=20), server_default="medium", nullable=False, comment="優先級"),
    ...
    op.create_index(op.f("ix_notifications_recipient_uuid"), "notifications", ["recipient_uuid"])
    op.create_index("ix_notifications_recipient_unread", "notifications", ["recipient_uuid", "read", "delete_at"])
...
def downgrade() -> None:
    ...
    op.drop_index(op.f("ix_notifications_recipient_uuid"), table_name="notifications")
