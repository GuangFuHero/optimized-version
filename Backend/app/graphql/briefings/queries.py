...
Briefings are internal deployment material: every read requires briefing.view (not public —
briefing.view is deliberately absent from PUBLIC_PERMS). check_permission gates each field.
"""
...
class BriefingQuery:
    ...
    async def briefing_templates(
        ...
        """List non-deleted templates, optionally filtered by state and/or tag. Requires briefing.view."""
        await check_permission(info, Perm.BRIEFING_VIEW)
    ...
    async def briefing_template(
        ...
        """Fetch a single non-deleted template by UUID. Requires briefing.view."""
        await check_permission(info, Perm.BRIEFING_VIEW)
    ...
    async def briefings(
        ...
        """List non-deleted briefings, optionally filtered by state and/or tag. Requires briefing.view."""
        await check_permission(info, Perm.BRIEFING_VIEW)
    ...
    async def briefing(
        ...
        """Fetch a single non-deleted briefing by UUID. Requires briefing.view."""
        await check_permission(info, Perm.BRIEFING_VIEW)
