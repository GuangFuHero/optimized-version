...
async def map_view_api(current_user: User = Depends(security.get_current_user)):
    ...
    """
    return {"status": "success", "message": f"驗證成功！使用者 {current_user.name} ({current_user.uuid}) 具有檢視地圖的權限。"}
...
async def map_create_api(current_user: User = Depends(security.get_current_user)):
    ...
    """
    return {"status": "success", "message": f"驗證成功！使用者 {current_user.name} ({current_user.uuid}) 具有新增地圖標記的權限。"}
...
async def admin_only_api(current_user: User = Depends(security.get_current_user)):
    ...
    """
    return {"status": "success", "message": f"驗證成功！使用者 {current_user.name} ({current_user.uuid}) 具有管理員權限。"}
