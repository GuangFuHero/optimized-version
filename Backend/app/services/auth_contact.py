...
async def _old_channel_proof(
    ...
    channel = _STEP_UP_CHANNEL_ZH.get(action, _DEFAULT_STEP_UP_CHANNEL_ZH)
...
# Where the code was delivered, for the same reason `_ACTION_ZH` exists (ADR-230): the 422
# has to describe what actually happened. Only a replacement or a removal acts *on* the
# channel it writes to; every other action sends to whichever contact the account already
# holds (`_proof_contact`), where "原聯絡方式" would be the wrong noun.
_STEP_UP_CHANNEL_ZH = {
    ACTION_REPLACE: "原聯絡方式",
    ACTION_REMOVE: "原聯絡方式",
}
_DEFAULT_STEP_UP_CHANNEL_ZH = "您帳號的聯絡方式"
