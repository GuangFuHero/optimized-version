...
gen_env() {     # .env is a deploy-time artifact: compose interpolation + container env share it (S7)
    ...
    [[ "$pg_pw" =~ ^[A-Za-z0-9_-]+$ ]] || die "app-postgres-password has .env/DSN-unsafe characters"
    [[ "$secret_key" =~ ^[A-Za-z0-9_-]+$ ]] || die "app-secret-key has .env-unsafe characters"
    [[ "$smtp_key" =~ ^[A-Za-z0-9_-]+$ ]] || die "app-smtp2go-api-key has .env-unsafe characters"
    umask 177
    {
...
main() {
    ...
    running_ctr=$(compose ps -aq backend | head -n1 || true)   # newest only; stale ones may linger
