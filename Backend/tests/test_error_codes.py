...
def test_no_auth_endpoint_still_raises_a_bare_http_exception():
    ...
    for path in sorted(_AUTH_ENDPOINTS.glob("*.py")):
        for node in ast.walk(ast.parse(path.read_text())):
            if not isinstance(node, ast.Raise) or not isinstance(node.exc, ast.Call):
                continue
            func = node.exc.func
            if getattr(func, "id", getattr(func, "attr", None)) == "HTTPException":
                offenders.append(f"{path.name}:{node.lineno}")

    assert offenders == [], f"raise HTTPException without a code: {', '.join(offenders)}"
