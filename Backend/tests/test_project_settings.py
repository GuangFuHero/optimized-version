...
async def test_a_typo_in_a_disaster_type_is_now_rejected(client, db_session):
    """`"floods"` for `"flood"` is refused outright since feature 018 closed the vocabulary.

    ADR-169 could only *warn* about this, because ADR-091 had left the vocabulary open and
    there was nothing to check a label against. Now there is: the label has to be a key in
    `disaster_types`. Rejecting beats warning here — a typo saved cleanly and then silently
    emptied every flood-scoped field out of the station and task forms.
    """
    admin_uuid = await _make_project_admin(db_session)
...
async def test_a_disaster_type_no_field_uses_comes_back_as_a_warning(client, db_session, caplog):
    ...
    This is what survives of ADR-169 now that the typo half is a hard rejection: `earthquake`
    is a perfectly valid key, but if every configured field is scoped to `flood`, selecting it
    empties the forms just as thoroughly. Configuring the disaster before its fields is a
    legitimate order to work in, so it saves — it just must not save *silently*.
    """
    admin_uuid = await _make_project_admin(db_session)
