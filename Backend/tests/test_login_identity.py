"""Endpoint tests for email-based login on the identity model."""
...
async def test_login_by_email(client, db_session):
    ...
    await create_password_account(
        db_session, email="a@x.com", password_hash=get_password_hash("secret", salt)
    )
...
async def test_login_wrong_password_401(client, db_session):
    ...
    # (a duplicate group makes group_repository.get_by_name raise MultipleResultsFound).
    await create_password_account(
        db_session, email="a@x.com", password_hash=get_password_hash("secret", generate_salt())
    )
...
async def test_login_unverified_contact_401(client, db_session):
    ...
    # Build the account by hand: create_password_account makes a VERIFIED contact, so we cannot use it.
