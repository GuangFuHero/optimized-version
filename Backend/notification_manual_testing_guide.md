...
**預期輸出**:
```text
======================== 16 passed, 4 warnings in 0.32s ========================
```

---

## 5. 資料保留清理任務實機驗證 (Retention worker live test)

驗證資料庫過期通知的軟刪除機制（PRD 第 9 節）：
1. **已讀超過 30 天** (`read_at < now - 30 days`) ➔ 設定 `delete_at = now()`
2. **建立超過 90 天** (`created_at < now - 90 days`) ➔ 設定 `delete_at = now()`
3. **未過期的新鮮通知** ➔ 完好保留 (`delete_at = None`)
...
```text
===========================================================================
📦 1. 成功向資料庫寫入 4 筆不同生命週期的測試通知：
   - N1: [過期已讀] 已讀 35 天前 ➔ 預期: 軟刪除 (delete_at != None)
   - N2: [過期未讀] 建立 95 天前 ➔ 預期: 軟刪除 (delete_at != None)
   - N3: [新鮮已讀] 已讀 10 天前 ➔ 預期: 保留   (delete_at == None)
   - N4: [新鮮未讀] 建立 5 天前  ➔ 預期: 保留   (delete_at == None)
===========================================================================

🚀 2. 執行 python3 scripts/run_retention_cleanup.py ...
2026-08-12 22:44:39,000 [INFO] 🚀 啟動通知資料保留清理任務 (Notification Retention Worker)...
2026-08-12 22:44:39,193 [INFO] ✅ 清理完成！共軟刪除 2 筆過期通知。

🔍 3. 檢查資料庫實際軟刪除與保留狀態：
   - N1: [過期已讀] 35天前物資更新          ➔ ❌ 已軟刪除 (delete_at = 2026-08-13 05:55:09)
   - N2: [過期未讀] 95天前緊急公告          ➔ ❌ 已軟刪除 (delete_at = 2026-08-13 05:55:09)
   - N3: [新鮮已讀] 10天前任務完成          ➔ ✅ 正常保留 (delete_at = None)
   - N4: [新鮮未讀] 5天前新工作分區          ➔ ✅ 正常保留 (delete_at = None)

===========================================================================
🎉 實機測試完全正確！過期的 2 筆被精確清理，新鮮的 2 筆完好保留！
===========================================================================
```
